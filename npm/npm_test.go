...
func testNpm(t *testing.T, when spec.G, it spec.S) {
	...
	// What do we do about package.json? Copy? Replace?
	// You don't need these to run a rebuild. But if they're metadata...what of that?
	when("RebuildLayer", func() {
		...
		it("returns an error when source does not exist", func() {
			mockRunner.EXPECT().Run(gomock.Any(), gomock.Any()).Times(0)
			err = Npm.RebuildLayer("/bad/path/node_modules", cacheLayer)
		...
		it("removes existing copies of the destination folder's `node_modules`", func() {
			...
			err = Npm.RebuildLayer(f.Build.Application.Root, cacheLayer)
			Expect(err).To(BeNil())
			Expect(filepath.Join(leftpad, "index.js")).ToNot(BeAnExistingFile())
		})

		it("copies the src directories node_modules folder", func() {
			leftpad := filepath.Join(f.Build.Application.Root, "node_modules", "left_pad")
			err = os.MkdirAll(leftpad, 0777)
			Expect(err).To(BeNil())
	...
	when("InstallInLayer", func() {
		...
		it("creates the cache layer if it does not exist", func() {
			mockRunner.EXPECT().Run(gomock.Any(), gomock.Any()).Times(1)

			err = Npm.InstallInLayer(f.Build.Application.Root, cacheLayer)
			Expect(err).To(BeNil())
			Expect(cacheLayer).To(BeAnExistingFile())
		})

		it("copies package-lock.json and package.json from app to cache layer", func() {
			mockRunner.EXPECT().Run(gomock.Any(), gomock.Any()).Times(1)
		...
		it("run NPM install in the cache layer", func() {
			installCommand := []string{"--prefix", cacheLayer, "install", "--unsafe-perm", "--cache", fmt.Sprintf("%s/npm-cache", cacheLayer)}
			mockRunner.EXPECT().Run(f.Build.Application.Root, installCommand).Times(1)
	...
	when("CopyToDst", func() {
		...
		it("copies modules from src to dst", func() {
			...
			err = Npm.CopyToDst(cacheLayer, f.Build.Launch.Root)
			Expect(err).To(BeNil())
